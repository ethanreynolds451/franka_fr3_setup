# Use the Franka API to exit FCI, lock the arm, and release the control token
...
# Use the API to exit FCI mode
def end_fci(token):
    url = f"https://{ROBOT_IP}/api/fci:deactivate"
    try:
        ...
        return True
    # Used for request-specific exceptions
    except requests.exceptions.RequestException as e:
        print(f"Error exiting FCI: {e}")
        return False

# Use the API to lock the arm joints
def lock_arm():
    # Doesn't require the token
    url = f"https://{ROBOT_IP}/api/arm/joints:lock"
    try:
        response = requests.post(
            url,
            auth=(USER, PASSWORD),
...
if __name__ == "__main__":
    ...
    if active_token:
        ...
        print("Exiting FCI...")
        if end_fci(active_token):
            print("FCI deactivated successfully.")
            print("Locking arm...")
            if lock_arm():
                print("Arm locked successfully.")
                ...
                print("Failed to lock arm. Attempting to release control token...")
                if release_token(active_token):
                    print("Control token released successfully.")
                    print("WARNING: arm is not locked. This can be completed through the DESK interface with physical access to the robot.")
                else:
                    print("Failed to release control token. Release can be forced through the DESK interface with physical access.")
        else:
            print("Failed to exit FCI. Please check the token and try again.")
    else:
        print("No control token provided. Attempting to lock arm without exiting FCI...")
        if lock_arm():
            print("Arm locked successfully.")
            print("You may need to take control through the DESK interface to release the token and exit FCI.")
        else:
            print("Failed to lock arm. This can be completed through the DESK interface.")
    print("Exiting in 10 seconds...")
    time.sleep(10)  # Give the user time to read the messages
