...
class FrankaKeyboardTrajectoryController(Node):
    ...
    def __init__(self):
        ...
        # List of gripper commands corresponding to listed inputs
        self.gripper_commands = ['rest', 'open', 'close']  # Example command types for gripper

        self.gripper_key_map = {
            'fr3_finger_joint1' : ['8', '0', '9'],
        }
        ...
        # Declare gripper control parameters
        self.gripper_increment = 0.005                                            # Define the increment for gripper position changes
        self.gripper_rest_position = [0.0]                                               # Define rest position for the gripper
        self.gripper_current_position = self.gripper_rest_position.copy()               # Initialize current position to rest
        self.gripper_rest_duration = 1.0                                       # How long to give the gripper to return to rest
        self.gripper_increment_duration = 0.1                                   # How long to give the gripper to execute an increment
        # Gripper finger 2 mirrors finger 1
        ...
        # Connect to the active action client for the gripper if specified

        self._gripper_action_client = ActionClient(
            self, 
            FollowJointTrajectory, 
            '/panda_gripper_trajectory_controller/follow_joint_trajectory'
        )
        ...
        for joint, keys in self.gripper_key_map.items():
            self.get_logger().info(f'{joint}: {keys[0]} (rest), {keys[1]} (open), {keys[2]} (close)')
    ...
    def on_key_press(self, key):
        ...
        for joint, keys in self.gripper_key_map.items():
            if key_char in keys:
                command = self.gripper_commands[keys.index(key_char)]  # Get the command type based on the key pressed
                self.update_gripper_trajectory(joint, command)  # Update the trajectory based on the command
                if command == 'rest':
                    self.send_gripper_trajectory_goal(self.gripper_current_position, duration_seconds=self.gripper_rest_duration)
                else: 
                    self.send_gripper_trajectory_goal(self.gripper_current_position, duration_seconds=self.gripper_increment_duration)
    ...
    def update_gripper_trajectory(self, joint_name, command):
        index = list(self.gripper_key_map.keys()).index(joint_name)  # Get the index of the joint to update
        if command == 'rest':
            self.gripper_current_position[index] = self.gripper_rest_position[index]  # Reset to rest position
        elif command == 'open':
            if self.gripper_current_position[index] + self.gripper_increment <= 0.04:  # Check upper limit
                self.gripper_current_position[index] += self.gripper_increment  # Increase joint angle
        elif command == 'close':
            if self.gripper_current_position[index] - self.gripper_increment >= 0:  # Check lower limit
                self.gripper_current_position[index] -= self.gripper_increment  # Decrease joint angle
    ...
    def send_gripper_trajectory_goal(self, target_positions, duration_seconds):
        # First check if the action server is available before sending a goal
        ...
        # Add joint names, must match the Franka yaml config
        joints = list(self.gripper_key_map.keys())
        goal_msg.trajectory.joint_names = joints
    ...
    def get_result_callback(self, future):
        ...
        # GripperCommand (effort) results may provide `success` or other fields.
        if hasattr(result_obj, 'success'):
            if not result_obj.success:
                self.get_logger().error('Gripper command reported failure.')
            else:
